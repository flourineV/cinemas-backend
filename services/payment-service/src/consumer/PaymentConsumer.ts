import { getChannel, PAYMENT_QUEUE, BOOKING_CREATED_KEY, BOOKING_FINALIZED_KEY, SEAT_UNLOCK_ROUTING_KEY, FNB_ORDER_CREATED_KEY } from '../config/rabbitConfig.js';
    ...
    public async startConsuming() {
        ...
        await channel.consume(PAYMENT_QUEUE, async (msg) => {
            ...
            try {
                ...
                console.log(`[PaymentConsumer] Received event | RoutingKey: ${routingKey}`);
                ...
                switch (routingKey) {
                    case BOOKING_CREATED_KEY: {
                        ...
                        console.log(`[PaymentConsumer] Processing BookingCreatedEvent | bookingId=${event.bookingId}`);
                        await this.paymentService.createPendingTransaction(event);
                        break;
                    }

                    case BOOKING_FINALIZED_KEY: {
                        const event: BookingFinalizedEvent = dataObj as BookingFinalizedEvent;
                        console.log(`[PaymentConsumer] Processing BookingFinalizedEvent | bookingId=${event.bookingId} | finalPrice=${event.finalPrice}`);
                    ...
                    case SEAT_UNLOCK_ROUTING_KEY: {
                        ...
                        console.log(`[PaymentConsumer] Processing SeatUnlockedEvent | bookingId=${event.bookingId} | showtimeId=${event.showtimeId} | seatIds=${event.seatIds}`);
                    ...
                    case FNB_ORDER_CREATED_KEY: {
                        ...
                        console.log(`[PaymentConsumer] Processing FnbOrderCreatedEvent | fnbOrderId=${event.fnbOrderId} | total=${event.totalAmount}`);
                        await this.paymentService.createPendingTransactionForFnb(event);
                        break;
                    }

                    default: {
                        console.warn(`[PaymentConsumer] Unknown routing key: ${routingKey}`);
                        break;
                    }
                }
                ...
                console.error(`[PaymentConsumer] Error processing message: ${(err as Error).message}`, err);
